 ...
 * @author mdames
 */
class GetEntryTts extends CliCmd {

    @Override
    String getName() {
        return("get-entry-tts");
    }

    @Override
    String getDescription() {
        return("get-entry-tts <oid>");
    }
    ...
    int execute(String[] args) {
        ...
        if(!CLI.getBook().getEntryByOID(oid).isTTS()) {
            return(error("OID ist nicht vom Typ TTS"));
        }
        
        System.out.println(CLI.getBook().getEntryByOID(oid).getTTS().text);
        return(ok());
    }
