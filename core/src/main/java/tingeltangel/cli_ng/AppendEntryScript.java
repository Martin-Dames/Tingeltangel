 ...
 * @author mdames
 */
class AppendEntryScript extends CliCmd {

    @Override
    String getName() {
        return("append-entry-script");
    }

    @Override
    String getDescription() {
        return("append-entry-script <oid> <script row>");
    }
    ...
    int execute(String[] args) {
        ...
        if(!CLI.getBook().getEntryByOID(oid).isCode()) {
            return(error("entry ist nicht von type skript"));
        }
